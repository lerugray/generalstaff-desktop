/**
 * Parse Claude Code `--output-format stream-json` usage for the live context meter (M3c).
 *
 ...
 *
 * Current context size for a turn = input + cache_read + cache_creation on the latest
 * assistant (or result) usage object. output_tokens are not part of the prompt window.
 *
 * No live capture existed in this workspace; the fixture is a protocol-faithful
 * reconstruction marked as such under test/fixtures/claude-stream-usage.jsonl.
 */
...
export interface ClaudeContextUsage {
  /** input + cache_read + cache_creation from the latest assistant/result usage. */
  usedTokens: number;
  usage: ClaudeUsageTokens;
  /** stream-json envelope type that carried the usage (`assistant` or `result`). */
...
export function parseClaudeStreamUsage(line: string): ClaudeContextUsage | undefined {
  ...
  if (type === 'assistant') {
    ...
    return {
      usedTokens: usage.inputTokens + usage.cacheReadTokens + usage.cacheCreationTokens,
      usage,
      source: 'assistant',
    };
  }
