...
The extension reads canonical project state, presents operator decisions and recent receipts, and streams work from existing Codex, Claude Fable, Kimi, Cline/GLM, Cursor, Grok 4.6 trial, and direct Ollama Cloud GLM 5.3 lanes. Tool steps and the finished pass show up as receipts: what happened, where (the project or file when we know it), and whether the work finished, stopped, or did not finish. A failed pass keeps that same card, so the problem is not buried in a stack. Seats sit on the desk as named instruments (Orchestrate, Build, Review, Verify, Fast assist) and show ready, thin, or down from the lanes that can take that job. The orchestrator seat keeps the model, effort, skill, and room-entry grant available; project orders retain their existing seat choices. Terminals and source files remain supporting instruments.
