...
The Codex lane explicitly selects GPT-5.6 Sol. Fable prefers an authenticated Claude CLI and safely falls back to an authenticated Cursor subscription with an explicit Fable 5 model. Grok 4.6 is an additional trial picker option backed by Cursor Agent. GLM 5.3 (Ollama) and GLM 5.3 Flash (Ollama) are additional read-only direct-API options backed by one authenticated Ollama Cloud catalog and adapter; the existing Cline / GLM lane is unchanged, and Fable remains the default orchestrator seat. The composer exposes only supported non-interactive effort values for Codex, Fable, Cline, and Grok; Kimi, Cursor, and Ollama use provider defaults. The rail also restores the six legacy GeneralStaff Desktop palettes and remembers the local selection.
